...
pub fn build(b: *std.Build) void {
    ...
    const zero_native_path = b.option([]const u8, "zero-native-path", "Path to the zero-native framework checkout") orelse resolveZeroNativePath(b);
    ...
    const dev = b.addSystemCommand(&.{ "npx", "--yes", "zero-native", "dev", "--manifest", "app.zon", "--binary" });
    ...
    const package = b.addSystemCommand(&.{
        "npx",           "--yes",                                                                                                                                  "zero-native",
        "package",       "--target",                                                                                                                               @tagName(package_target),
        "--manifest",    "app.zon",                                                                                                                                "--assets",
        "frontend/dist", "--optimize",                                                                                                                             optimize_name,
        "--output",      b.fmt("zig-out/package/{s}-0.1.0-{s}-{s}{s}", .{ app_exe_name, @tagName(package_target), optimize_name, packageSuffix(package_target) }), "--binary",
    });
...
fn resolveZeroNativePath(b: *std.Build) []const u8 {
    if (b.graph.environ_map.get("ZERO_NATIVE_PATH")) |path| {
        if (path.len > 0) return path;
    }

    if (scanNpxZeroNativePath(b)) |path| return path;

    const install = std.process.run(b.allocator, b.graph.io, .{
        .argv = &.{ "npx", "--yes", "zero-native", "--version" },
        .stdout_limit = .limited(4096),
        .stderr_limit = .limited(4096),
    }) catch {
        @panic("Unable to resolve zero-native. Set ZERO_NATIVE_PATH=/path/to/zero-native or install Node.js/npx.");
    };
    defer b.allocator.free(install.stdout);
    defer b.allocator.free(install.stderr);
    if (install.term != .exited or install.term.exited != 0) {
        @panic("Unable to install zero-native with npx. Set ZERO_NATIVE_PATH=/path/to/zero-native.");
    }

    if (scanNpxZeroNativePath(b)) |path| return path;
    @panic("npx completed but zero-native was not found in the npx cache. Set ZERO_NATIVE_PATH=/path/to/zero-native.");
}

fn scanNpxZeroNativePath(b: *std.Build) ?[]const u8 {
    const script =
        \\node -e "
        \\const path=require('path'),fs=require('fs');
        \\const d=path.join(require('os').homedir(),'.npm','_npx');
        \\if(!fs.existsSync(d))process.exit(1);
        \\let best=null,mt=0;
        \\for(const e of fs.readdirSync(d)){
        \\  const p=path.join(d,e,'node_modules','zero-native','package.json');
        \\  if(fs.existsSync(p)){const m=fs.statSync(p).mtimeMs;if(m>mt){mt=m;best=path.dirname(p);}}
        \\}
        \\if(best){process.stdout.write(best);process.exit(0);}
        \\process.exit(1);
        \\" 2>/dev/null
    ;
    ...
    if (platform == .macos) {
        switch (web_engine) {
            ...
            .chromium => {
                ...
                if (cef_auto_install) {
                    const cef_auto = b.addSystemCommand(&.{ "npx", "--yes", "zero-native", "cef", "install", "--dir", cef_dir });
